...
const navigation = [
  ...
  { to: "/enrollments", label: "Pengelolaan KRS", icon: BookOpen },
  { to: "/students", label: "Mahasiswa", icon: GraduationCap },
  { to: "/courses", label: "Mata Kuliah", icon: Library },
];

export default function Sidebar({ open, onClose }: Props) {
  return (
    <>
      {open && <button type="button" aria-label="Tutup navigasi" onClick={onClose} className="fixed inset-0 z-40 bg-gray-950/35 backdrop-blur-[1px] lg:hidden" />}
      <aside className={`fixed inset-y-0 left-0 z-50 flex w-[272px] transform flex-col border-r border-slate-800 bg-slate-950 text-white transition-transform duration-200 lg:sticky lg:top-0 lg:h-screen lg:translate-x-0 ${open ? "translate-x-0" : "-translate-x-full"}`}>
        <div className="flex h-20 items-center justify-between border-b border-slate-800 px-6">
          ...
          <button type="button" onClick={onClose} aria-label="Tutup menu samping" className="rounded-lg p-2 text-slate-400 hover:bg-slate-800 hover:text-white lg:hidden"><X className="h-5 w-5" /></button>
        </div>

        <nav aria-label="Navigasi utama" className="flex-1 px-4 py-6">
          <p className="px-3 text-[10px] font-semibold uppercase tracking-[0.18em] text-slate-500">Menu Utama</p>
        ...
        <div className="m-4 border-t border-slate-800 px-3 pt-4">
          <p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-slate-500">Sistem Informasi Akademik</p>
          <p className="mt-2 text-xs leading-5 text-slate-400">KRS, mahasiswa, dan mata kuliah dalam satu ruang kerja.</p>
        </div>
      </aside>
    </>
  );
